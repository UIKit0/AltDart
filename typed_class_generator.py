...
from dcc import dt_parse
from config import Glob, glob
from typed_classes import resolve_type
...
def gen_class_code(node, typespace):
  ...
  #build vtable
  vcode = "void *" + node.name + "__vtable[] = {\n"
  
  #constructor always goes first
  for f in vtable:
    if f[0] != "constructor": continue
    vcode += f[1]
  ...
  vcode += "};\n\n"
  ret.prepend(CppCode(vcode))
  
  #build struct
  scode = "typedef struct " + node.name + " {\n"
  scode += "  TypeStruct *__type__;\n";
  
  for p in node.props.values():
    scode += "  " + p.get_cdecl_str() + ";\n"
  scode += "} " + node.name + ";\n\n"
  
  ret.prepend(CppCode(scode))
  
  #type object
  tcode = "TypeStruct " + node.name + "__type = {\n"
  tcode += "  " + node.name + "__vtable,\n"
  tcode += "  " + node.name + "__ref_offsets, //for garbage collection; implement me! \n"
  tcode += "};\n\n"
  
  ret.prepend(CppCode(tcode))
...
class TransformVisit (NodeVisit):
  ...
  def CFunctionNode(self, node, scope, t, tlevel):
    scope = dict(scope)
  ...
  def var_transform(self, cls, var, member):
    ...
    if type(p) == VarDeclNode:
      ...
      off = p.start
      ret = dt_parse(var.gen_js(0) + "[%d][%d];" % (arr, off), start_node=ArrayRefNode)
      ret.type = p
      ...
      if member in cls.getters:
        ...
        ret = dt_parse(name+"($n)", [var], start_node=FuncCallNode)
