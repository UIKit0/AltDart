from lex import HexInt, StringLit
from config import glob
from util_types import odict
...
class Node:
  ...
  def get_line_str(self):
    name = str(type(self)).replace("ast.", "").replace("<class", "").replace(">", "").replace(" ", "").replace("'", "")
  ...
  def get_ntype_name(self):
    return str(type(self)).replace("ast.", "").replace("<class", "").replace(">", "").replace(" ", "").replace("'", "")
...
class VarDeclNode(IdentNode):
  ...
  def get_type_str(self):
    def get_type(n, visit=None):
      ...
      if type(n) == str:
        ...
        if type(n) in [VarDeclNode, IdentNode]:
          return n.val
        else:
          return "(corruption)"
      elif type(n) == IdentNode:
        return n.val
      elif type(n) == StaticArrNode:
        return "%s[%i]" % (get_type(n[0], visit), n.size)
  ...
  def get_cdecl_str(self):
    ret = self.get_type_str()
    
    #handle arrays
    if "[" in ret:
      base = ret[:ret.find("[")]
      arr = ret[ret.find("["):ret.rfind("]")+1]
      ret = "%s %s%s" % (base, self.val, arr)
    else:
      ret += " " + self.val
